...
<div class="row">
    ...
    <div class="col-md-3 pull-right">
      <button class="btn btn-md btn-orange pull-right" data-toggle="modal" @click="openDefectModal()"><i class="fa fa-plus-circle" aria-hidden="true"></i> Raise Defect</button>
    </div>
...
<!-- </form> -->
    ...
    <div class="row bg-white paddx15 pdt-10 mgt-5" v-for="(defect,index) in defectsList">
      <div class="col-md-1">
      ...
      <div class="col-md-4">
       ...
       <button v-bind:style="{background: label.defectLabelColourCode}" @click="searchDefect(label,index)"> {{label.defectLabelName}}</button></span>
      </div>
      <div class="col-md-2">
      ...
      <!-- <div class="col-md-1 text-center">
        <a type="button"  class="cursor-pointer"><span class="cmmnIcon color-orange"><i aria-hidden="true" class="fa fa-pencil" @click="editDefectModal(defect,index)"></i></span></a>
      </div> -->
    </div>
</div>
...
export default{
  ...
  data(){
    return{
      ...
      comment:'',
      index:'',
      projectId: this.$store.getters.getDevProjectInfo.projectId,
      selectedEpic:'',
    ...
    methods:{
      ...
      getLabels: function() {
          ...
          app.$http.get(
              app.$store.state.SERVER_URL + '/api/dev-pm-admin/get-defect-labels-in-project/'+this.$store.getters.getDevProjectInfo.projectId
              , {
                  headers: {
      ...
      searchDefect(label){
        ...
        app.recordPerPage = ''
        app.showData = 'hidden';
       app.search.projectId =   app.$store.getters.getDevProjectInfo.projectId
      ...
      getAllStoryandEcpics: function(){
          ...
          app.$store.commit('setSpinner',true);
        this.$http.get(
          app.$store.state.SERVER_URL+'/api/dev-pm-admin/get-qa-defects-epics-storys-testscenarios-testcases-in-project/'+app.$store.getters.getDevProjectInfo.projectId,
        {
          headers: {
            "X-Authorization": app.$cookie.get('X-Authorization')
          },
