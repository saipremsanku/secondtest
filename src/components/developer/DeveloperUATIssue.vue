               ...
               <div class="col-md-12">
                    ...
                    <div class='form-group paddx15'>
                      <select class="form-control" v-model="selectedStory" name="selectedStory">
                        ...
                        <option v-for="story in storyList" :value="story">{{story.name}}</option>
                      </select>
                    </div>
              ...
              <div class="col-xs-12">
                <form data-vv-scope="Defect-comment-scope" >
                  <div class="form-group paddx15" :class="{'textarea': true, 'is-danger': errors.has('Defect-comment-scope.comment') }">
                    <textarea class="form-control" placeholder="Add comments here" rows="3" name="comment" v-model="comment" v-validate data-vv-rules="required"  disabled="true"></textarea>
...
export default{
  ...
  methods:{
    ...
    deleteFile: function(selectedEpic){
      var app = this;
      app.selectedStory = ''
      app.storyList = []
      app.storyList =  (selectedEpic && selectedEpic.userStorys) ? selectedEpic.userStorys:'';
...
//               app.defectFiles=(response.data.payLoad);
//      }, (response) => {
//    });
//  }else{
   ...
   getNextPage(pageNumber){
    ...
    getDefectList: function(){
      var app =this;
         app.$store.commit('setSpinner',true);
      this.$http.get(
        app.$store.state.SERVER_URL+'/api/po-dev-pm-admin/get-uat-defects-in-project/'+app.$store.getters.getDevProjectInfo.projectId,
      {
        headers: {
          "X-Authorization": app.$cookie.get('X-Authorization')
        },
    ...
    getAllStoryandEcpics: function(){
      ...
      app.onloadPagination = '';
         app.$store.commit('setSpinner',true);
      this.$http.get(
        app.$store.state.SERVER_URL+'/api/po-dev-pm-admin/get-epics-and-stories-in-project/'+app.$store.getters.getDevProjectInfo.projectId,
      {
        headers: {
          "X-Authorization": app.$cookie.get('X-Authorization')
        },
